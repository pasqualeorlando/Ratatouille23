...
const ClientiServiti = (props) => {
  ...
  function calcolaClientiServiti(dipendenteSelezionato, dataInizio, dataFine, dipendenti) {
    let clientiServiti = 0;
    let clientiTotali = 0;
    ...
    dipendenti.forEach(dipendente => {
      dipendente.ordinazioni?.forEach(ordinazione => {
        ...
        if(dataOrdinazione.getTime() >= dataI.getTime() && dataOrdinazione.getTime() <= dataFinale.getTime()) {
          if(dipendente.idDipendente === dipendenteSelezionato.idDipendente)
            clientiServiti += ordinazione.tavolo.numeroOspiti;
          clientiTotali += ordinazione.tavolo.numeroOspiti;
        }
      })
    })
    return [clientiTotali, clientiServiti];
    
  }

  const [clientiTotali, clientiServiti] = calcolaClientiServiti(dipendenteSelezionato, dataInizio, dataFine, dipendenti);
    ...
    datasets: [
      {
        ...
        data: dipendenteSelezionato.length === 0 ? [] : [(clientiServiti * 100 / clientiTotali) || 0, 100 - ((clientiServiti * 100 / clientiTotali) || 0)],
  ...
  return (
    <Box sx={{bgcolor: 'background.paper', boxShadow: '0px 3px 7px', p: 4, width: '60%',
    '@media screen and (max-width: 800px)': {width: '100%'}, borderRadius: '30px'}}>
        <Typography variant="h5" component="h5"><b>Clienti serviti</b></Typography>
